...
API_BASE = "https://api.tabtouch.com.au/v1"
API_HEADERS = {
...
def _fetch_all_meetings() -> Tuple[List[Dict], List[Dict]]:
    ...
    try:
        r = client.get(
            ...
            params={"date": today, "jurisdiction": "WA"},
        )
        if r.status_code != 200:
            logger.warning(f"TABtouch API returned {r.status_code}, will retry on next request")
            return ([], [])
        data = r.json()
        meetings = data.get("meetings", data.get("data", []))
    except Exception as e:
        logger.error(f"Failed to fetch TABtouch meetings: {e}, will retry on next request")
        return ([], [])
    ...
    for m in meetings:
        ...
        category = (m.get("raceType", m.get("category", "") or "").upper())
        ...
        for race in races:
            ...
            if race_num == 0:
                continue
            try:
                event_id = race.get("id", race.get("eventId", race.get("raceId", "")))
                if not event_id:
                    continue
    ...
    logger.info(f"TABtouch: fetched {len(jockey_markets)} jockey and {len(driver_markets)} driver meetings")
    return _all_cache
        ...
        if m.get("meetingName", m.get("name", "")).lower().strip() == meeting_name.lower().strip():
            for race in m.get("races", m.get("events", [])):
                if race.get("raceNumber", race.get("race_number", 0)) == race_number:
                    race_id = race.get("id", race.get("eventId", race.get("raceId", "")))
                    break
            break
